...
@export var dialog_timeline1: String = "exactlyion_boy1"
@export var dialog_timeline1s: String = "exactlyion_boy1s"
@export var dialog_timeline2: String = "exactlyion_boy2"
@export var dialog_timeline2s: String = "exactlyion_boy2s"
@export var dialog_timeline3: String = "exactlyion_boy3"
@export var dialog_timeline3c: String = "exactlyion_boy3m"
@export var dialog_timeline3cs: String = "exactlyion_boy3ms"
	...
	if not visible:
		if Global.timeline < 6:
			...
			if Global.route_status == "Magus":
				if Global.screwdriver_on_boy:
					visible = true
					animation_player.play("idle2c2")
				elif not Global.screwdriver_on_boy:
					visible = true
					animation_player.play("idle2c")
			else:
				visible = true
...
func _start_dialog():
	...
	if Global.timeline < 6:
		if not Global.has_screwdriver and not Global.screwdriver_on_boy:
			Dialogic.start(dialog_timeline1, false)
		elif Global.has_screwdriver and not Global.screwdriver_on_boy:
			Dialogic.start(dialog_timeline1s, false)
			Global.screwdriver_on_boy = true
			Global.has_screwdriver = false
			Global.affinity += 2
	# Add to player inventory
			#if "inventory" in Global.player.body:
			Global.player.remove_item_from_inventory("Screwdriver")
				#body.inventory.append(inventory_id)
		elif Global.screwdriver_on_boy and not Global.has_screwdriver:
			Dialogic.start(dialog_timeline1s, false)
			
	elif Global.timeline >= 6 and Global.timeline < 10:
		if Global.route_status == "Magus":
				if Global.screwdriver_on_boy:
					Dialogic.start(dialog_timeline3cs, false)
				elif not Global.screwdriver_on_boy:
			...
			if not Global.has_screwdriver and not Global.screwdriver_on_boy:
				...
				Dialogic.start(dialog_timeline2s, false)
				Global.screwdriver_on_boy = true
				Global.has_screwdriver = false
				Global.affinity += 2
		# Add to player inventory
