...
class_name DarkMonsterFlamethrower
...
enum State { IDLE, ALERT, PREPARE, FIRING, ENDING }
...
func _process(delta: float) -> void:
	...
	match state:
		...
		State.PREPARE:
			# Only cancel if player disappears; transition to FIRING is via _on_body_animation_finished("prepare")
			if not player_visible:
				_set_state(State.IDLE)
...
func _on_flame_animation_finished() -> void:
	...
	if state == State.FIRING and anim_name == "start":
		...
		# After ending anim, go back to idle fully
		flame_sprite.visible = false
		_set_state(State.IDLE)
