...
@export var pickup_id: String = "Sword"  # Set unique per instance in inspector
@export var bounce_force := 100.0
@export var inventory_id := "Sword"
@export var float_height := 5.0
@export var float_speed := 0.5 * Global.global_time_scale  


@export var chip_texture: Texture2D = preload("res://assets_image/Objects/Objects4.png")
...
func _ready():
	# Physics
	if Global.has_sword or Global.sword_on_boy:
		print("Sword: already collected (", pickup_id, "), despawning.")
		queue_free()
		return
	...
	if has_node("CollectionArea"):
		...
		push_error("Sword: Missing CollectionArea child!")
