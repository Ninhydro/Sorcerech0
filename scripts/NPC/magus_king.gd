...
@onready var sprite_2dV: Sprite2D = $Varek_king
@onready var sprite_2dZ: Sprite2D = $Zach
@onready var animation_playerV: AnimationPlayer = $Varek_king/AnimationPlayer
@onready var animation_playerZ: AnimationPlayer = $Zach/AnimationPlayer
@onready var interaction_area: Area2D = $InteractionArea
@onready var bubble_timer: Timer = $BubbleTimer

# Configuration
#@export var dialog_timeline1: String = "betael1"
#@export var dialog_timeline2: String = "varek2"
@export var dialog_timeline3V: String = "varek_king3"
@export var dialog_timeline3Z: String = "zach3"
...
func _ready():
	...
	visible = false
	sprite_2dV.visible = false
	sprite_2dZ.visible = false
	# Start bubble timer
	_reset_bubble_timer()
...
func _process(delta):
	...
	if not Global.timeline >= 10:
		return
	
	# Make visible when condition is met
	if not visible and Global.timeline >= 10:
		if Global.alyra_dead == false:
			visible = true
			sprite_2dV.visible = false
			sprite_2dZ.visible = true
			animation_playerZ.play("idle")
		elif Global.alyra_dead == true:
			visible = true
			sprite_2dV.visible = true
			sprite_2dZ.visible = false
			animation_playerV.play("idle")
	
	# Flip sprite based on player position
	if Global.player:
		sprite_2dV.flip_h = Global.player.global_position.x < global_position.x
		sprite_2dZ.flip_h = Global.player.global_position.x < global_position.x
...
func _start_dialog():
	...
	if Global.timeline >= 10 and Global.alyra_dead == false:
		Dialogic.start(dialog_timeline3Z, false)
	elif Global.timeline >= 10 and Global.alyra_dead == true:
		Dialogic.start(dialog_timeline3V, false)
