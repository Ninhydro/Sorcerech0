...
@export var pickup_id: String = "Glasses"  # Set unique per instance in inspector
@export var bounce_force := 100.0
@export var inventory_id := "Glasses"
@export var float_height := 5.0
@export var float_speed := 0.5 * Global.global_time_scale  


@export var chip_texture: Texture2D = preload("res://assets_image/Objects/Objects2.png")
...
func _ready():
	# Physics
	if Global.has_glasses or Global.glasses_on_man:
		print("Glasses: already collected (", pickup_id, "), despawning.")
		queue_free()
		return
	...
	if has_node("CollectionArea"):
		...
		push_error("Glasses: Missing CollectionArea child!")
...
func _on_body_entered(body):
	...
	#print("Player collected Glasses! Total:", Global.persistent_microchips)
	queue_free()
