...
@onready var sprite_2d: Sprite2D = $Valentina
@onready var animation_player: AnimationPlayer = $Valentina/AnimationPlayer
@onready var interaction_area: Area2D = $InteractionArea
@onready var bubble_timer: Timer = $BubbleTimer

# Configuration
@export var dialog_timeline1: String = "valentina1"
@export var dialog_timeline2: String = "valentina2"
@export var dialog_timeline3: String = "valentina3"
...
func _process(delta):
	...
	if not Global.timeline >= 4:
		return
	
	 #Make visible when condition is met
	if not visible and Global.timeline >= 4 and Global.minigame_valentina_completed:
		visible = true
		animation_player.play("idle")
	elif Global.route_status == "Magus":
		visible = false
