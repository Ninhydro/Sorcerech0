...
@export var dialog_timeline1: String = "aerendale_oldman1"
@export var dialog_timeline1g: String = "aerendale_oldman1g"
@export var dialog_timeline1g2: String = "aerendale_oldman1g2"
...
func _process(delta):
	...
	if not (Global.timeline >= 3 and Global.timeline <= 6):
		return
	
	# Make visible when condition is met
	if not visible and (Global.timeline >= 3 and Global.timeline <= 6):
		visible = true
		animation_player.play("idle")
		if Global.glasses_on_man:
...
func _start_dialog():
	...
	if  (Global.timeline >= 3 and Global.timeline <= 6):
		if not Global.has_glasses and not Global.glasses_on_man:
			Dialogic.start(dialog_timeline1, false)
		elif Global.has_glasses and not Global.glasses_on_man:
			Dialogic.start(dialog_timeline1g, false)
			Global.glasses_on_man = true
			Global.has_glasses = false
			Global.affinity -= 2
	# Add to player inventory
			#if "inventory" in Global.player.body:
			Global.player.remove_item_from_inventory("Glasses")
				#body.inventory.append(inventory_id)
		elif Global.glasses_on_man and not Global.has_glasses:
			Dialogic.start(dialog_timeline1g2, false)
