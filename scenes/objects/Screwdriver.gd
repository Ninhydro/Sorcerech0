...
@export var pickup_id: String = "Screwdriver"  # Set unique per instance in inspector
@export var bounce_force := 100.0
@export var inventory_id := "Screwdriver"
@export var float_height := 5.0
@export var float_speed := 0.5 * Global.global_time_scale  


@export var chip_texture: Texture2D = preload("res://assets_image/Objects/Objects3.png")
...
func _ready():
	# Physics
	if Global.has_screwdriver or Global.screwdriver_on_boy:
		print("Screwdriver: already collected (", pickup_id, "), despawning.")
		queue_free()
		return
	...
	if has_node("CollectionArea"):
		...
		push_error("Screwdriver: Missing CollectionArea child!")
...
func _on_body_entered(body):
	...
	if not Global.has_screwdriver:
		Global.has_screwdriver = true
